...
const useDeleteStage = (queryParams) => {
  ...
  const qry = `/${idCompanyAccess}/process-types/${idProcessType}/stages/${stageSelected.id}`

  return useMutation({
    mutationFn: async () => {
      const response = await request(base, qry, 'delete', null, null)
