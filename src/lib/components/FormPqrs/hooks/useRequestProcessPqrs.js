import { MagicString, useGetProcess, useMutateProcess } from '@/lib'
import toast from 'react-hot-toast'

const useRequestProcessPqrs = ({
  idProcess,
  idGroup,
  activeStep,
  setActiveStep,
  ...
  const { mutateAsync: createProcess, isPending: loadingCreationProcess } = useMutateProcess({
    ...
    onSuccess: async (response) => {
      ...
      setTimeout(() => {
        navigate(`/inbox/reqPqrs?idGroup=${idGroup}&idProcess=${idProcess}&edition=true`)
      }, 500)
    },
    onError: (e) => {
      toast.error(e?.response?.data?.error ?? MagicString.GENERAL.ERROR_MESSAGE)
...
export default useRequestProcessPqrs
