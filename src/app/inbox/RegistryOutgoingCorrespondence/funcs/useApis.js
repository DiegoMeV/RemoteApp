...
const useApis = ({ setActiveStep, activeStep, idProcess, setConfirmModalProcess }) => {
  ...
  const { mutateAsync: createProcess } = useMutationDynamicBaseUrl({
    ...
    onSuccess: (e) => {
      ...
      navigate(`/inbox/RegistryOutgoingCorrespondence?idProcess=${e.data?.id}`)
      setTimeout(() => {
  ...
  const loadingProcess =
    loadingInfoProcess || loadingActivityUpdate || loadingActivityUpdate || isPendingGenerateSticker
