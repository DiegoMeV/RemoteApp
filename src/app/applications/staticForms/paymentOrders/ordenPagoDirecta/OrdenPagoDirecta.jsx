...
import { ViewOrdenPagoDirecta } from './views'
import { useStoreState } from 'easy-peasy'
import { QUERY_ORDEN_PAGO } from './components/Master/funcs'
import { FORM_GLOBAL_VARIABLES } from './funcs'
import { useCommonFuncs } from './hooks'

const OrdenPagoDirecta = () => {
  ...
  const {
    data: ordenPagoData,
    isLoading: fisrtLoadingOrdenPagoData,
    isFetching: loadingOrdenPagoData,
    refetch: refetchOrdenPago,
    isError: errorLoadingOrdenPagoData,
  } = useQueryOracle({
    query: QUERY_ORDEN_PAGO({
      nit_compania,
      queryParamsPks: orderedQueryParams?.pks,
      queryParams: orderedQueryParams?.nonPks,
    }),
    enabled: !!orderedQueryParams?.pks?.orden,
    queryKey: ['orden_pago', nit_compania, orderedQueryParams?.pks?.orden],
  })
  ...
  const newGlobalVariables = {
    ...
    division2: userParameters?.ver_todas_op === 'S' ? '%' : globalVariables?.division,
    ...newGlobals,
  }
  ...
  const getFechaBLQ = async () => {
    const fecha = ordenPagoData?.data?.[0]?.fecha
  ...
  return (
    ...
    >
      <LoadingError
        loading={fisrtLoadingOrdenPagoData || loadingFormGV}
        error={errorLoadingOrdenPagoData}
      >
        <ViewOrdenPagoDirecta
          form={form}
...
export default OrdenPagoDirecta
